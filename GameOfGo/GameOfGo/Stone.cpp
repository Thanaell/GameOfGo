...
//TODO: les kos et la prise autorisée entre 4 pierres ennemies + le bug bizarre
bool Stone::createStone(int x, int y, StoneColor color, Board& myBoard) {
	...
	std::shared_ptr<Stone> myStone(new Stone(x, y, color, myBoard));
	...
	if (isStonePlaced) {
		...
		//s'il y a 1 à 4 pierres de même couleur adjacentes
			...
			std::map<std::shared_ptr<Formation>,int> oldFormations;
			for (auto it : neighbours) {
				if (it != nullptr && it->myColor == color) {
					oldFormations.insert({ it->formation,0 });
				}
			}
			//calcul des libertés de la potentielle nouvelle formation
			int newLiberties = 0;
			for (auto it : oldFormations) {
				newLiberties += it.first->getLiberties() - 1;
			}
